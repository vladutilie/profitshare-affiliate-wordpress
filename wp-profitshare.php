...
Plugin URI: https://wordpress.org/plugins/wp-profitshare
...
add_action( 'plugins_loaded', 'ps_i18n' );
add_shortcode( 'tag_image', 'ps_tag_image_shortcode' );

function ps_i18n() {
	load_plugin_textdomain( 'wp-profitshare', FALSE, basename( dirname( __FILE__ ) ) . '/languages/' );
}
...
function ps_enqueue_admin_assets() {
	...
	if ( ! empty( $screen->id ) && 'profitshare_page_ps_keywords_settings' == $screen->id ) {
		wp_enqueue_script( 'profitshare-admin-script', plugins_url( 'js/admin.js', __FILE__ ), array( 'jquery' ) );
	} elseif ( ! empty( $screen->id ) && 'profitshare_page_ps_tag_image' == $screen->id && isset( $_REQUEST['do'] ) ) {
		wp_enqueue_media();
	...
	} elseif ( ! empty( $screen->id ) && 'profitshare_page_ps_history_links' == $screen->id && isset( $_REQUEST['batch'] ) ) {
...
function ps_footer_js() {
	...
	if ( ! empty( $post->id ) ) {
		...
		if ( 'none' == $limit_keyword_links ) {
			return;
		} elseif ( $limit_keyword_links > 0 ) {
			$sql_limit = $limit_keyword_links;
		}
	}
	$links = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}ps_keywords LIMIT $sql_limit" );
	if ( ! empty( $links ) ) {
		?>
			...
			jQuery().ready( function( e ) {
				...
				foreach ( $links as $link ) {
					...
					if ( $link->tip_display == 'y' ) {
						$newText = 'light' == $link->tip_style ? '<a class="pslinks ttlight"' : '<a class="pslinks ttdark"';
						$newText .= ' href="' . $link->link . '" target="' . $openin . '">' . $link->keyword . '<span><div class="ttfirst"></div><strong>' . $link->tip_title . '</strong><br />';
						if ( $link->tip_image != '' ) {
							$newText .= '<img src="' . $link->tip_image . '" alt="CSS tooltip image" style="float:right;width:90px;margin:0 0 10px 10px;" />';
						}
						$newText .= $link->tip_description . '<div class="ttlast">' . __( 'WP Profitshare', 'wp-profitshare' ) . ' ' . PS_VERSION . '</div></span></a>';
					} else {
...
function ps_add_menus() {
	...
	add_submenu_page( 'ps_account_settings', __( 'Plugin Settings', 'wp-profitshare' ), __( 'Plugin Settings', 'wp-profitshare' ), 'delete_posts', 'ps_account_settings', 'ps_account_settings' );
	$current_user = wp_get_current_user();
	if ( get_user_meta( $current_user->ID, 'ps_is_api_connected', true ) ) {
		add_submenu_page( 'ps_account_settings', __( 'Keyword Settings', 'wp-profitshare' ), __( 'Keyword Settings', 'wp-profitshare' ), 'delete_posts', 'ps_keywords_settings', 'ps_keywords_settings' );
		add_submenu_page( 'ps_account_settings', __( 'Conversions', 'wp-profitshare' ), __( 'Conversions', 'wp-profitshare' ), 'delete_posts', 'ps_conversions', 'ps_conversions' );
		#add_submenu_page( 'ps_account_settings', __( 'Campaigns', 'wp-profitshare' ), __( 'Campaigns', 'wp-profitshare' ), 'delete_posts', 'ps_campaigns', 'ps_campaigns' );
		add_submenu_page( 'ps_account_settings', __( 'Tag image', 'wp-profitshare' ), __( 'Tag image', 'wp-profitshare' ), 'delete_posts', 'ps_tag_image', 'ps_tag_image' );
		add_submenu_page( 'ps_account_settings', __( 'Link history', 'wp-profitshare' ), __( 'Link history', 'wp-profitshare' ), 'delete_posts', 'ps_history_links', 'ps_history_links' );
		add_submenu_page( 'ps_account_settings', __( 'Help', 'wp-profitshare' ), __( 'Help', 'wp-profitshare' ), 'delete_posts', 'ps_useful_info', 'ps_useful_info' );
	}
}
...
function ps_account_settings() {
	...
	if ( isset( $_POST['connect'] ) ) {
		if ( $is_api_connected ) {
			$alert['type'] = 'success';
			$alert['text'] = __( 'Connected: API data is correct and the connection has been successful established.', 'wp-profitshare' );
		} elseif ( ! $is_api_connected ) {
			$alert['type'] = 'error';
			$alert['text'] = __( 'Disconnected: API data is incorrect or connection error occurred.', 'wp-profitshare' );
		} 
	}
	?>
	...
	<div class="wrap">
		<div class="ps-logo">
			<a href="<?php echo config( 'PS_HOME' ); ?>"><img src="https://profitshare.ro/assets/img/logos/_logo-menu-profitshare.svg" alt="Profitshare" /></a>
		</div><!-- .ps-logo -->
		<div class="row">
			<div class="col-md-10 col-xs-12 col-md-push-1 ps-shadow">
				<h2 class="ps-h text-center ps-margin"><?php _e( 'Profitshare plugin options', 'wp-profitshare' ); ?></h2>
				<div class="row">
					...
					if ( $is_api_connected ) {
						...
						<div class="col-md-4 col-xs-12">
							<div class="ps-option mt9">
								<h3 class="ps-h3 ps-no_margin_top"><?php _e( 'Profitshare links in posts', 'wp-profitshare' ); ?></h3>
								<form action="" method="post" style="display:inline;">
									...
									<input type="checkbox" name="links_in_posts" <?php echo $posts_status ? 'checked' : ''; ?> data-on-text="<?php _e( 'On', 'wp-profitshare' ); ?>" data-off-text="<?php _e( 'Off', 'wp-profitshare' ); ?>" data-on-color="success" data-off-color="danger" data-label-width="30" data-handle-width="40" data-size="small" />
								</form>
								<form action="<?php echo admin_url( 'admin.php' ); ?>" method="get" style="display:inline;">
									...
									<input type="submit" name="submit" class="btn btn-primary ps-btn ps-generate" value="<?php _e( 'Generate', 'wp-profitshare' ); ?>" />
								</form>
							</div><!-- .ps-option -->

							<div class="ps-option">
								<h3 class="ps-h3 ps-no_margin_top"><?php _e( 'Profitshare links in pages', 'wp-profitshare' ); ?></h3>
								<form action="" method="post" style="display:inline;">
									<input type="checkbox" name="links_in_pages" <?php echo $pages_status ? 'checked' : ''; ?> data-on-text="<?php _e( 'On', 'wp-profitshare' ); ?>" data-off-text="<?php _e( 'Off', 'wp-profitshare' ); ?>" data-on-color="success" data-off-color="danger" data-label-width="30" data-handle-width="40" data-size="small" />
								</form>
								<form action="<?php echo admin_url( 'admin.php' ); ?>" method="get" style="display:inline;">
									<input type="hidden" name="page" value="ps_history_links" />
									<input type="hidden" name="batch" value="posts" />
							...
							<div class="ps-option">
								<h3 class='ps-h3 ps-no_margin_top'><?php _e( 'Profitshare links in comments', 'wp-profitshare' ); ?></h3>
								<form action="" method="post" style="display:inline;">
									<input type="checkbox" <?php echo ( $comment_status ) ? 'checked' : ''; ?> name="links_in_comments" data-on-text="<?php _e( 'On', 'wp-profitshare' ); ?>" data-off-text="<?php _e( 'Off', 'wp-profitshare' ); ?>" data-on-color="success" data-off-color="danger" data-label-width="30" data-handle-width="40" data-size="small" />
								</form>
								<form action="<?php echo admin_url( 'admin.php' ); ?>" method="get" style="display:inline;">
									<input type="hidden" name="page" value="ps_history_links" />
						...
						<div class="col-md-4 col-xs-12">
							<div class="ps-alerts">
								<div class="ps-alert ps-info-alert">
									<?php _e( 'By clicking on "Generate" all your direct links from all your posts/comments published so far will be converted into Profitshare affiliate links.', 'wp-profitshare' ); ?>
								</div><!-- .ps-alert -->
								<div class="ps-alert ps-info-alert">
									<?php _e( 'By turning On all the links from your future posts/comments will be automatically converted into Profitshare affiliate links.', 'wp-profitshare' ); ?>
								</div><!-- .ps-alert -->
								<div class="ps-alert ps-info-alert ps-danger-alert">
									<?php _e( 'We recommend making a backup of you database before running this functionality.', 'wp-profitshare' ); ?>
								</div><!-- .ps-alert -->
							</div><!-- .ps-alerts -->
						</div><!-- .col-md-4 -->
						<?php
...
function ps_keywords_settings() {
	...
	<div class="wrap">
		...
		<h2 class="ps-h ps-margin"><?php _e( 'Keyword Settings', 'wp-profitshare' ); ?></h2>
		<a href="<?php echo admin_url( 'admin.php?page=ps_keywords_settings&do=add' ); ?>" class="btn btn-success"><?php _e( 'Add keyword', 'wp-profitshare' ); ?></a>
		<?php if ( ! empty( $_REQUEST['do'] ) && $_REQUEST['do'] != 'delete' ) : ?>
			<a class="btn btn-primary" href="<?php echo admin_url( 'admin.php?page=ps_keywords_settings' ); ?>"><?php _e( 'Show keywords list', 'wp-profitshare' ); ?></a>
		<?php endif;
		$show_table = true;
		$errors = array();
		$success = array();
			...
			switch ( $_REQUEST['do'] ) {
				case 'delete':
					if ( ! empty( $_REQUEST['keyword_id'] ) ) {
						...
						if ( $do_query) {
							$success[] = __( 'Keyword deleted.', 'wp-profitshare' );
						} else {
							$errors[] = __( 'Keyword delete error.', 'wp-profitshare' );
						}
					}
					break;
				case 'edit':
					if ( ! empty( $_REQUEST['keyword_id'] ) && ! empty( $_POST ) ) {
						if ( '' == $_POST['keyword'] ) {
							$errors[] = __( '<strong>Keyword</strong> required.', 'wp-profitshare' );
						}
						if ( '' == $_POST['title'] ) {
							$errors[] = __( '<strong>Title</strong> required.', 'wp-profitshare' );
						}
						if ( '' == $_POST['link'] ) {
							$errors[] = __( '<strong>Link </strong> required.', 'wp-profitshare' );
						}
						if ( '' != $_POST['link'] && ! preg_match( '#^https?://profitshare\.ro/l/#i', $_POST['link'] ) && config( 'PS_HOME' ) == 'http://profitshare.ro' ) {
							$errors[] = sprintf( __( '<strong>Your link</strong> must start with %1$s/l/', 'wp-profitshare' ),
											config( 'PS_HOME' )	// %1$s
										);
						}
						...
						if ( '' == $_POST['openin'] ) {
							$errors[] = __( '<strong>Open link in...</strong> required.', 'wp-profitshare' );
						}
						if ( '1' == $_POST['tip_display'] ) {
							if ( '' == $_POST['tip_title'] ) {
								$errors[] = __( '<strong>Tooltip title</strong> required.', 'wp-profitshare' );
							}
							if ( '' == $_POST['tip_description'] ) {
								$errors[] = __( '<strong>Tooltip description</strong> required.', 'wp-profitshare' );
							}
						}
						if ( empty( $errors ) ) {
							...
							$wpdb->update(
								...
								array( 'ID' => $_POST['ID'] ), array( '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', ), array( '%d' )
							);
							$success[] = __( 'Keyword saved.', 'wp-profitshare' );
						}
					}
					$show_table = false;
					break;
				case 'add':
					if ( ! empty( $_POST ) ) {
						...
						if ( ( strpos( $_POST['link'], 'profitshare.ro' ) === false ) && config( 'PS_HOME' ) == 'http://profitshare.ro' ) {
							$errors[] = sprintf( __( '<strong>Your link</strong> must contain %1$s.', 'wp-profitshare' ),
								config( 'PS_HOME' ) // %1$s
							);
						}
					...
					if ( ! empty( $_REQUEST['keyword_id'] ) ) {
						...
						<form method="post">
							<table class="form-table ps-form-list">
								<tbody>
									<tr valign="top">
										<th scope="row"><label for="keyword"><?php _e( 'Keyword', 'wp-profitshare' ); ?></label></th>
										<td>
											<input class="large-text ps-input" type="text" name="keyword" value="<?php echo ! empty( $_POST['keyword'] ) ? $_POST['keyword'] : $row['keyword']; ?>" />
										</td>
									</tr>
									<tr valign="top">
										<th scope="row"><label for="title"><?php _e( 'Title', 'wp-profitshare' ); ?></label></th>
										<td>
											<input class="large-text ps-input" type="text" name="title" value="<?php echo ! empty( $_POST['title'] ) ? $_POST['title'] : $row['title']; ?>" />
										</td>
									</tr>
									<tr valign="top">
										<th scope="row"><label for="link"><?php _e( 'Link', 'wp-profitshare' ); ?></label></th>
										<td>
											<input class="large-text ps-input" type="text" name="link" value="<?php echo ! empty( $_POST['link'] ) ? $_POST['link'] : $row['link']; ?>" />
										</td>
									</tr>
									<tr valign="top">
										<th scope="row"><label for="openin"><?php _e( 'Open link in', 'wp-profitshare' ); ?></label></th>
										<td>
											<select name="openin" class="ps-select">
												<option value="new"<?php echo ( ! empty( $_POST['openin'] ) && 'new' == $_POST['openin'] ) || 'new' == $row['openin'] ? ' SELECTED' : ''; ?>><?php _e( 'New window or tab (_blank)', 'wp-profitshare' ); ?></option>
												<option value="current"<?php echo ( ! empty( $_POST['openin'] ) && 'current' == $_POST['openin'] ) || 'current' == $row['openin'] ? ' SELECTED' : ''; ?>><?php _e( 'Same window or tab (_none )', 'wp-profitshare' ); ?></option>
											</select>
										</td>
									</tr>
									<tr valign="top" class="ps-checkbox">
										<th scope="row"><label for="tip_display"><?php _e( 'Tooltip', 'wp-profitshare' ); ?></label></th>
										<td>
											<label><input name="tip_display" id="tip_display" type="radio" value="y" onclick="toggleTips(1);"<?php echo ( ! empty( $_POST['tip_display'] ) && 'y' == $_POST['tip_display'] ) || 'y' == $row['tip_display'] ? ' checked="checked"' : ''; ?>><?php _e( 'Yes', 'wp-profitshare' ); ?></label>
											<label><input name="tip_display" id="tip_display" type="radio" value="n" onclick="toggleTips(0 );"<?php echo ( ! empty( $_POST['tip_display'] ) && 'n' == $_POST['tip_display'] ) || 'n' == $row['tip_display'] ? ' checked="checked"' : ''; ?>><?php _e( 'No', 'wp-profitshare' ); ?></label>
										</td>
									</tr>
									<tr valign="top" class="tip_display_1 hide_display ps-checkbox">
										<th scope="row"><label for="tip_style"><?php _e( 'Design tooltip', 'wp-profitshare' ); ?></label></th>
										<td>
											<label><input name="tip_style" type="radio" value="light"<?php echo ( ! empty( $_POST['tip_style'] ) && 'light' == $_POST['tip_style'] ) || 'light' == $row['tip_style'] ? ' checked="checked"' : ''; ?>><?php _e( 'Light', 'wp-profitshare' ); ?></label>
											<label><input name="tip_style" type="radio" value="dark"<?php echo ( ! empty( $_POST['tip_style'] ) && 'dark' == $_POST['tip_style'] ) || 'dark' == $row['tip_style'] ? ' checked="checked"' : ''; ?>><?php _e( 'Dark', 'wp-profitshare' ); ?></label>
										</td>
									</tr>
									<tr valign="top" class="tip_display_1 hide_display">
										<th scope="row"><label for="tip_title"><?php _e( 'Tooltip title', 'wp-profitshare' ); ?></label></th>
										<td>
									...
									<tr valign="top" class="tip_display_1 hide_display">
										<th scope="row"><label for="tip_description"><?php _e( 'Tooltip description', 'wp-profitshare' ); ?></label></th>
										<td>
									...
									<tr valign="top" class="tip_display_1 hide_display ps-checkbox">
										<th scope="row"><label for="tip_image"><?php _e( 'Tooltip image', 'wp-profitshare' ); ?></label></th>
										<td>
											...
											<input class="button-primary upload_image_button" data-id="1" type="button" value="<?php _e( 'Select image', 'wp-profitshare' ); ?>" />
										</td>
									</tr>
									<tr valign="top">
										<th scope="row">
						...
						<table class="form-table">
							<tbody>
								...
								<tr valign="top">
									...
									<td>
										<select name="openin" class="ps-select">
											<option value="new"<?php echo ! empty( $_POST['openin'] ) && 'new' == $_POST['openin'] ? ' SELECTED' : ''; ?>><?php _e( 'New window or tab (_blank)', 'wp-profitshare' ); ?></option>
											<option value="current"<?php echo ! empty( $_POST['openin'] ) && 'current' == $_POST['openin'] ? ' SELECTED' : ''; ?>><?php _e( 'Same window or tab (_none )', 'wp-profitshare' ); ?></option>
										</select>
									</td>
								</tr>
								<tr valign="top">
									<th scope="row"><label for="tip_display"><?php _e( 'Tooltip', 'wp-profitshare' ); ?></label></th>
									<td>
										<label><input name="tip_display" id="tip_display" type="radio" value="y" onclick="toggleTips(1);"<?php echo ! empty( $_POST['tip_display'] ) && 'y' == $_POST['tip_display'] ? ' checked="checked"' : ''; ?>><?php _e( 'Yes', 'wp-profitshare' ); ?></label>
										<label><input name="tip_display" id="tip_display" type="radio" value="n" onclick="toggleTips(0 );"<?php echo ! empty( $_POST['tip_display'] ) && 'n' == $_POST['tip_display'] ? ' checked="checked"' : ''; ?><?php echo empty( $_POST ) ? ' checked="checked"' : ''; ?>><?php _e( 'No', 'wp-profitshare' ); ?></label>
									</td>
								</tr>
								<tr valign="top" class="tip_display_1 hide_display">
									<th scope="row"><label for="tip_style"><?php _e( 'Design tooltip', 'wp-profitshare' ); ?></label></th>
									<td>
										<label><input name="tip_style" type="radio" value="light"<?php echo ! empty( $_POST['tip_style'] ) && 'light' == $_POST['tip_style'] ? ' checked="checked"' : ''; ?>><?php _e( 'Light', 'wp-profitshare' ); ?></label>
										<label><input name="tip_style" type="radio" value="dark"<?php echo ! empty( $_POST['tip_style'] ) && 'dark' == $_POST['tip_style'] ? ' checked="checked"' : ''; ?><?php echo empty( $_POST ) ? ' checked="checked"' : ''; ?>><?php _e( 'Dark', 'wp-profitshare' ); ?></label>
									</td>
								</tr>
								<tr valign="top" class="tip_display_1 hide_display">
									<th scope="row"><label for="tip_title"><?php _e( 'Tooltip title', 'wp-profitshare' ); ?></label></th>
...
function ps_campaigns() {
	...
	<div class="wrap">
		...
		<h2 class="ps-h"><?php _e( 'Advertisers campaigns', 'wp-profitshare' ); ?></h2>
	</div>
	<!-- generate campaign url -->
	<?php
	ps_update_campaigns();
	if ( isset( $_GET['generate_campaign_url'] ) && $_GET['generate_campaign_url'] ) {
		$campaign_id = absint( $_GET['generate_campaign_url'] );
		...
		if ( $campaign_data ) {
			...
			// create url
			if ( ! $affiliate_url) {
				$ps_shorten_link = ps_shorten_link( sprintf( __( 'WP Profitshare - Campaign %1$s', 'wp-profitshare' ), $campaign_data['name'] ), $campaign_data['url'] );
		...
		if ( $campaign_data ) {
			...
			<div class="campaign_details">
				<div class="campaign_name">
					<b><?php _e( 'Campaign name:', 'wp-profitshare' ); ?></b> <span><?php echo $campaign_data['name']; ?></span>
				</div>
				<div class="advertiser_name">
					<b><?php _e( 'Advertiser:', 'wp-profitshare' ); ?></b> <span><?php echo $campaign_data['advertiser_id']; ?></span>
				</div>
				<div class="campaign_url">
					<b><?php _e( 'Campaign URL:', 'wp-profitshare' ); ?></b> <span><?php echo $campaign_data['url']; ?></span>
				</div>
				<?php if ( isset( $campaign_data['ps_url'] ) && $campaign_data['ps_url'] ) : ?>
					<div class="ps_url">
						<b><?php _e( 'PS URL:', 'wp-profitshare' ); ?></b> <span><?php echo $campaign_data['ps_url']; ?></span>
					</div>
				<?php endif; ?>
				<div class="status">
					<b><?php _e( 'Status:', 'wp-profitshare' ); ?></b> <span><?php echo $campaign_data['status']; ?></span>
				</div>
				<div class="start_date">
					<b><?php _e( 'Start date:', 'wp-profitshare' ); ?></b> <span><?php echo $campaign_data['start_date']; ?></span>
				</div>
				<div class="end_date">
					<b><?php _e( 'End date:', 'wp-profitshare' ); ?></b> <span><?php echo $campaign_data['end_date']; ?></span>
				</div>
				<?php if ( isset( $campaign_data['banners'] ) && $campaign_data['banners'] ) : ?>
					<div class="banners">
						<b><?php _e( 'Campaign banners:', 'wp-profitshare' ); ?></b> 
	...
	<script>
		...
		$( '.copy-btn' ).on( 'click', function() {
			...
			if ( copy_to_clipboard( copy_content ) ) {
				alert(<?php _e( 'Successfully copied to clipboard!', 'wp-profitshare' ); ?>);
			}
		});
...
function ps_conversions() {
	...
	<div class="wrap">
		...
		<h2 class="ps-h"><?php _e( 'Conversions', 'wp-profitshare' ); ?></h2>
		<div class="row">
			<div class="col-md-5 col-xs-12 current_earnings">
				<div class="ps-earnings-content col-md-6 col-xs-12 text-center">
					<div><?php _e( 'Your current earnings are', 'wp-profitshare' ); ?></div>
				...
				<div class="col-md-6 col-xs-12">
					...
					if ( isset( $_POST['update_conversions'] ) ) {
						...
						<div class="ps-alert ps-white">
							<?php _e( 'Your conversions has been updated with success!', 'wp-profitshare' ); ?>
						</div>
						<?php
					}
					?>
					<form method="post" action="">
						...
						<?php if ( $data_last_update ) : ?>
							<div class="ps-info"><b><?php _e( 'Last update:', 'wp-profitshare' ); ?></b> <?php echo $data_last_update; ?></div>
						<?php endif; ?>
					</form>
				</div>
			</div>
						...
						<tr>
							<td class="p0">
								<div class="ps-button-sticky-to-input ps-right">
									...
									<input type="submit" name="submit_link" class="ps-new-btn ps-green ps-inline" value="<?php _e( 'Get link', 'wp-profitshare' ); ?>" />
									<div class="ps_checkbox_big">
										<input type="checkbox" data-on-text="<?php _e( 'HTTP', 'wp-profitshare' ); ?>" data-off-text="<?php _e( 'HTTPS', 'wp-profitshare' ); ?>" name="link_type" data-on-color="info" data-off-color="primary" data-label-width="30" data-handle-width="40" data-size="small">
									</div>
								</div>
							</td>
						</tr>
						<tr>
							...
							if ( isset( $_POST['submit_link'] ) ) {
								...
								if ( ! $ps_shorten_link['result'] ) {
									echo '<div class="ps-alert ps-right ps-error ps-error-big"><span>' . __( 'An error occurred or link is not part of Profitshare and can not be generated.', 'wp-profitshare' ) . '</span></div>';
								} else {
									?>
									<!--</div>-->
									<td class="td-padding">
										<div class="ps-generated-link ps-right">
											<div class="ps-button-sticky-to-input ps-right">
												...
												<input type="submit" name="submit_link" class="ps-new-btn ps-green ps-inline" id="copy_link" value="<?php _e( 'Copy', 'wp-profitshare' ); ?>" />
											</div>
										</div>
									</td>
									<?php
	...
	function copyToClipboard( text ) {
		...
		document.execCommand( 'copy' );
		$temp.remove();
		alert( <?php _e( 'Link copied to clipboard!', 'wp-profitshare' ); ?> );
	}
...
function ps_tag_image() {
	...
	<div class="wrap">
		...
		<h2 class="ps-h ps-margin"><?php _e( 'Tag image', 'wp-profitshare' ); ?></h2>
		<a href="<?php echo admin_url( 'admin.php?page=ps_tag_image&do=add' ); ?>" class="btn btn-success"><?php _e( 'Add image', 'wp-profitshare' ); ?></a>
		<?php if ( ! empty( $_REQUEST['do'] ) && $_REQUEST['do'] != 'delete' ) {
			echo '<a class="btn btn-primary" href="' . admin_url( 'admin.php?page=ps_tag_image' ) . '">' . __( 'Show images list', 'wp-profitshare' ) . '</a>';
		}
		$show_table = true;
		$errors = array();
		$success = array();
			...
			switch ( $_REQUEST['do'] ) {
				case 'delete':
					if ( ! empty( $_REQUEST['tag_id'] ) ) {
						...
						if ( $do_query) {
							$success[] = __( 'Image deleted.', 'wp-profitshare' );
						} else {
							$errors[] = __( 'Image delete error.', 'wp-profitshare' );
						}
					}
					break;
				case 'edit':
					if ( ! empty( $_REQUEST['tag_id'] ) && ! empty( $_POST ) ) {
						...
						if ( '' == $_POST['image'] ) {
							$errors[] = __( '<strong>Image</strong> required.', 'wp-profitshare' );
						}
						if ( empty( $errors ) ) {
							$wpdb->update(
								$table_name,
							...
							$success[] = __( 'Tag image saved.', 'wp-profitshare' );
						}
					}
					$show_table = false;
					break;
						...
						<form method="post">
							<table class="form-table">
								<tbody>
									...
									<tr valign="top">
										<th scope="row"><label for="image"><?php _e( 'Image', 'wp-profitshare' ); ?></label></th>
										<td>
											<input id="upload-img" type="button" class="button" value="Upload"/> <input id="uploaded-img-clear" type="button" class="button" value="<?php _e( 'Delete tags', 'wp-profitshare' ); ?>"/>
											<br /><br />
...
function ps_history_links_list() {
	...
	<div class="wrap">
		...
		<h2 class="ps-h ps-margin"><?php _e( 'Link history', 'wp-profitshare' ); ?></h2>
...
function ps_history_links_batch() {
	...
	<div class="wrap">
		...
		<?php
		printf( __( '<h2>Generate Profitshare links for %1$s</h2>', 'wp-profitshare' ),
			'<span class="batch_type">' . $_GET['batch'] . '</span>' // %1$s
		);
		_e( '<p>Please do not refresh the page until the process is done, otherwise the process will start from the begining.</p>', 'wp-profitshare' );
		printf( __( '<p>Total items to process: %1$s</p>', 'wp-profitshare' ),
			'<span class="batch_count">' . $count_items . '</span>' // %1$s
		);
		printf( __( '<p>Processed items: %1$s</p>', 'wp-profitshare' ),
			'<span class="batch_processed">0</span>' // %1$s
		);
		?>
		<div id="progressbar"><div class="plabel"><?php _e( 'Loading...', 'wp-profitshare' ); ?></div></div>
...
function ps_useful_info() {
	...
	<div class="wrap">
		<a href="<?php echo config( 'PS_HOME' ); ?>"><img src="<?php echo plugin_dir_url(__FILE__ ); ?>/images/profitshare-conversion.png" alt="Profitshare" /></a><br />
		<hr style="border-top: 1px dashed #7f8c8d;">
		<h2><?php _e( 'Frequently Asked Questions', 'wp-profitshare' ); ?></h2><br />
		<strong><?php _e( 'What is Profitshare?', 'wp-profitshare' ); ?></strong>
		<ul>
			<li><?php _e( 'Profitshare is an affiliate marketing network, that is, a performance driven marketing tool.', 'wp-profitshare' ); ?></li>
		</ul>
		<strong><?php _e( 'What is an advertiser?', 'wp-profitshare' ); ?></strong>
		<ul>
			<li><?php _e( 'The advertiser is the one that wants to have its products or services advertised on the internet in order to obtain clients and increase its revenue.', 'wp-profitshare' ); ?></li>
		</ul>
		<strong><?php _e( 'What is an affiliate?', 'wp-profitshare' ); ?></strong>
		<ul>
			<li><?php _e( 'The affiliate is the one that advertises products and services offered by advertisers, through agreed methods, and earns a percentage of each sale that is made as a result of this.', 'wp-profitshare' ); ?></li>
		</ul>
		<strong><?php _e( 'Who is the client?', 'wp-profitshare' ); ?></strong>
		<ul>
			<li><?php _e( 'The client is the one that reaches advertisers\' websites thanks to affiliate promotion. This client may then perform a pre-established action on the advertiser\'s website like: making a purchase, subscribing to a newsletter, signing up for an account etc.', 'wp-profitshare' ); ?></li>
			<li><?php _e( 'As <strong>an affiliate</strong> you gain access to a wide range of brands and varied products and services that can satisfy any kind of advertising projects, using efficient affiliate marketing tools. As <strong>an advertiser</strong>, the biggest advantage is acquiring an extensive community of ambassadors for your products or services from the affiliate marketing network', 'wp-profitshare' ); ?></li>
		</ul>
		<strong><?php _e( 'What is Profitshare for affiliates?', 'wp-profitshare' ); ?></strong>
		<ul>
			<li><?php _e( 'The <strong>Profitshare affiliate plugin</strong> is a tool that helps you grow your conversion rate and gain money online. Profitshare for affiliates lets you see your conversion history, facilitates generating affiliate links automatically or manually whenever you publish a new post. You can also watch your current earnings directly from your WordPress interface and you can replace al your existing links with affiliate links so that you can earn money with all your previous posts.', 'wp-profitshare' ); ?></li>
		</ul>
	</div>
	<?php
}
